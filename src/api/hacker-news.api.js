...
const getNews = async (cursor = 0, limit = 10, category) => {
  const { data: newsIds } = await axios.get(
    `${baseURL}/${apiVersion}/${category}.json?print=pretty`
  );

  const index = cursor > newsIds.length ? newsIds.length - limit : cursor;

  const news = await Promise.all(
    newsIds?.slice(index, index + limit)?.map((id) => getItem(id))
  );

  return news.map(({ data: { title, url } }) => ({ title, url }));
};
